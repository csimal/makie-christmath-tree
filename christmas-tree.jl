...
# ╔═╡ a89327ec-a1eb-11ee-1d4f-a390b2548e94
begin
	using GLMakie
	using Colors
	using Random
...
# ╔═╡ acee12f8-4faf-4171-b1b2-d63193f8f13a
function barnsleysfern(A, b, p, n=1_000_000)
	cp = cumsum(p)
	x = zeros(2,n)
	for i in 2:n
		k = searchsortedfirst(cp, rand())
		x[:,i] .= A[k] * x[:,i-1] + b[k]
	end
	return x
end

# ╔═╡ 238a4044-5146-43c6-a37d-5ae12f57b6a9
begin
	A = [
		[0.0 0.0; 0.0 0.16],
		[0.85 0.0; 0.0 0.85],
...
# ╔═╡ 36b295ff-bcaf-4a94-ac32-ad33d2eb01d2
x = barnsleysfern(A, b, p)

# ╔═╡ 5d117d95-79cc-41ec-bbde-ea57024d1b25
points = Point2f.(eachcol(x))

# ╔═╡ 517e1ba4-ac0a-4b5e-9d6b-0f7909079d24
begin
	c = to_colormap(:Greens)
	c[2:end] = reverse(c[2:end])
	c[1:end-2]
end

# ╔═╡ 0040ab70-7f91-49e7-bbd4-99ad53d45d1e
rand(points, 20)

# ╔═╡ 64298cb4-bea6-4615-8366-fe38c58d339a
summit = argmax(p -> p[2], points)

# ╔═╡ 480357c0-f247-47ef-a5e3-32dd8ad45382
function radius(θ, a, n, e)
	a * cos(n*θ) / (1 + e*cos(n*θ))
end

# ╔═╡ 37872150-5962-4ce4-b7f6-3917378693fd
begin
	θ = LinRange(0.0,2π, 300)
	ρ = radius.(θ, 1.0, 6, 0.5)
	θ_lights = LinRange(0.0,2π,13)
	ρ_lights = radius.(θ_lights, 1.0, 6, 0.5)
	x_star = ρ .* cos.(θ)
	y_star = ρ .* sin.(θ)
	x_starlights = ρ_lights .* cos.(θ_lights)
	y_starlights = ρ_lights .* sin.(θ_lights)
	f = lines(x_star, y_star)
	scatter!(x_starlights, y_starlights, color = :red)
	f
end

# ╔═╡ 9632ff16-e05d-4f39-8add-fb43fbcf4b11
begin
	...
	hidedecorations!(ax)
	hidespines!(ax)
	cmap = to_colormap(:Greens)
	cmap[1] = to_color(:purple4)
	cmap[2:end] .= reverse(cmap[2:end])
	datashader!(points, colormap = cmap[1:end-2])
	#scatter!(points[1:10000], color = :green)
	ts = LinRange(1.8, 10.0, 100)
	lines!(-3.5 .* sin.(2.5*ts .+ 1.0) .* exp.(-0.2 .* ts), ts,
		linewidth = 3.0,
		color = :firebrick)
	nballs = 40
	balls = rand(filter(p -> p[2] < 9.5, points), nballs)
	nlights = 120
	lights = rand(filter(p -> p[2] < 10.0, points), nlights)
	scatter!(lights,
		color = :lightyellow,
		markersize = 3,
		glowwidth = 5.0,
		glowcolor = :goldenrod1
	)
	ball_colors = [Colors.JULIA_LOGO_COLORS.red, Colors.JULIA_LOGO_COLORS.blue]
	scatter!(balls, 
		color = rand(1:15, nballs), 
		markersize = rand(20:25,nballs), 
		colormap = ball_colors,
	)
	
	lines!(summit[1] .+ 0.1 .* x_star, summit[2] .+ 0.2 .* y_star, color = :gold)
	scatter!(summit[1] .+ 0.1 .* x_starlights, summit[2] .+ 0.2 .* y_starlights,
		color = :lightyellow,
		markersize = 4,
		glowwidth = 3,
		glowcolor = :goldenrod1
	)
	text!(-3.0, 9.5, text = "Merry\n Christmath", 
		color = :white,
		fontsize = 24,
		font = "Tostada"
	)
	text!(2.2, 10.0, text = "2023", 
		color = :white,
		fontsize = 24,
		font = "Tostada"
	)
...
# ╔═╡ 7007b33c-71a2-4353-a7a8-d7d457ec7e70
fig

# ╔═╡ 1146aac4-91b3-47e9-8869-83559faa2c72
save("christmath_tree.png", fig)
...
julia_version = "1.9.4"
manifest_format = "2.0"
project_hash = "17fb42389df263a70d1731bb3e9f0d38fb3712f1"
...
git-tree-sha1 = "6d21f3720d7fb69e287d70314d97109464731431"
